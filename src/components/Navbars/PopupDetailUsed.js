...
class PopupDetailUsed extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    const { data, onConfirm, onUnConfirm, isShow, onClose, onChangeReason } = this.props;
        ...
        <div className="modal-body text-default-custom">
          <div style={{
            ...
            {data.IsPrint === false ?
              ...
              <i>Doanh nghiệp/Hợp tác xã yêu cầu được tự in tem</i>
            }
          </div>
          <br />
          ...
          <div className='modal-body-item-2'>
            ...
            <label className='modal-body-item-label'>- Phiếu yêu cầu cấp phát tem có chữ ký của giám đốc (nếu là doanh nghiệp), hoặc chữ ký cá nhân</label> */}
              {
                dataConfig && dataConfig.attachmentUsed ? (
                  <>
                    {/* <div dangerouslySetInnerHTML={{ __html: dataConfig.attachmentUsed.split(`\r\n`).join('<br/>') || '' }}></div> */}
                    <div dangerouslySetInnerHTML={{ __html: dataConfig.attachmentUsed || '' }} className="css-p-margin"></div>
                  </>
                ) : null
              }
            ...
            {(data.RequestedUsedStatus == 1 || data.RequestedUsedStatus == 3) ?
              <div className='modal-body-item-2'>
                <label className='css-label-stamp-provide'>Lý do không duyệt</label>
