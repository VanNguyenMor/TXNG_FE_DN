...
import { PARTNER_VERIFY } from "../../../helpers/constant";
...
class PartnerVerify extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
      ...
      if (typeof (data) !== 'undefined') {
        if (typeof (data.listVerify) !== 'undefined') {
          if (data.listVerify !== null) {
            if (typeof (data.listVerify.partners) !== 'undefined') {
              data.listVerify.partners.map((item, key) => {
              ...
              this.setState({
                data: data.listVerify.partners,
                history: data.listVerify.partners,
                listLength: data.listVerify.total,
                totalPage: Math.ceil(data.listVerify.partners.length / limit),
                isLoaded: data.isLoading,
                status: data.status,
                message: PLEASE_CHECK_CONNECT(data.message),
                totalElement: data.listVerify.total > LIMIT_ITEM_IN_PAGE ? LIMIT_ITEM_IN_PAGE : data.listVerify.total
              });
            } else {
              this.setState({
                history: data.listVerify.partners,
  ...
  fetchSummary = (data) => {
    const { requestListPartnerVerify } = this.props;

    requestListPartnerVerify(data);
  }
                  ...
                  <Row>
                    <div className="col">
                      ...
                      <Card className="shadow">
                        <Table className="align-items-center tablecs table-css-partnerVerify" responsive >
                                ...
                                data
                                  ...
                                  .map((item, key) => (
                                    <tr key={key} className="table-hover-css">
                                      ...
                                      <td style={{ textAlign: 'center' }} className='table-scale-col partnerVerify-img'>{item.thumbnail}</td>
