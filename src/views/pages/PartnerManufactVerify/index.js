...
import { PARTNER_MANUFACT_VERIFY } from "../../../helpers/constant";
...
import './PartnerManufactVerify.css';
...
class PartnerManufactVerify extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
      ...
      headerTitle: PARTNER_MANUFACT_VERIFY,
  ...
  handleUploadProducts = () => {
    ...
    requestUploadFilePartner(formData).then(res => {
      if (res.data.status == 200) {
        ...
        this.toggleModal('updateModal');
        toast.success('Upload hồ sơ nhà sản xuất thành công!')
      } else {
    ...
    if (idCurRow) {
      requestDeleteFilePartner(idCurRow).then((res) => {
        if (res.data.status == 200) {
          ...
          toast.success('Huỷ hồ sơ nhà sản xuất thành công!')
        } else {
          this.setState({ errMesageFail: res.message });
          this.toggleModal('popupMessage')
        }
    ...
    if (idCurRowVerify) {
      requestVerifyPartner(idCurRowVerify).then(res => {
        if (res.status == 200) {
          ...
          this.toggleModal('updateModal2');
          toast.success('Xác thực nhà sản xuất thành công!')
        } else {
          this.setState({ errMesageFail: res.message });
          this.toggleModal('popupMessage')
        }
                                ...
                                data
                                  ...
                                  .map((item, key) => (
                                    <tr key={key} className="table-hover-css ">
                                      ...
                                      <td style={{ textAlign: 'center' }} className='table-scale-col css-img-partner-manufact'>{item.thumbnail}</td>
                                      ...
                                      <td style={{ textAlign: 'left', whiteSpace: 'normal' }}>{item.verifiedDate ? moment(item.verifiedDate).format('DD/MM/YYYY') : null}</td>
