...
class InsertOrUpdate extends Component {
  ...
  async componentDidMount() {
    ...
    const { onHandleChangeValue, batchId } = props;

    // If batch mode (batchId provided), don't call legacy onHandleChangeValue
    if (!batchId && onHandleChangeValue) {
  ...
  onChangeSelect = (name) => (value) => {
    ...
    const selectedOption = props.DIARY_OPTIONS?.find(
    ...
    this.setState(
      ...
      () => {
        // Only call legacy callback if not in batch mode
        if (!props.batchId && props.onHandleChangeValue) {
          props.onHandleChangeValue(this.state);
        }
      }
    );
  };
