...
import { REPORT_ZONING_PLANTING_ZONE } from "../../../helpers/constant";
...
class ReportZoningPlantingZone extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: null,
            district: null,
            ward: null,
            headerTitle: REPORT_ZONING_PLANTING_ZONE,
            plantingZoneId: '',
            districtId: '',
            wardId: '',
            fromDate: '',
            toDate: '',
            page: 0,
            limit: LIMITS.reportV2ListZoningPlantingZone,
    ...
    onFilter = () => {
        const { fromDate, toDate, plantingZoneId, districtId, wardId, page, limit } = this.state;
        const { getReportListZoningPlantingZone } = this.props;
        ...
        getReportListZoningPlantingZone(body).then(res => {
    ...
    onExcel = () => {
        const { fromDate, toDate, plantingZoneId, districtId, wardId, page, limit } = this.state;
        const { printExcelReportListZoningPlantingZone } = this.props;
        ...
        printExcelReportListZoningPlantingZone(body).then(res => {
            ...
            if (ok) {
                ...
                if (data) {
                    fakeDownloadLinkFile(data, FILE_NAMES.reportV2ListZoningPlantingZone());
            ...
            <>
                {
                    <div className={classes.wrapper}>
                        <Container fluid>
                                    ...
                                    <div className="row" style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        ...
                                        <div>
                                            ...
                                            <Select
                                                ...
                                                ref={ref => this.redSelectWard = ref}
                                                data={ward}
                                                labelName='wardName'
                                                val='id'
                            ...
                            <h3 className={`${classes.text} ${classes.textLable}`}> BÁO CÁO QUY HOẠCH VÙNG SẢN XUẤT </h3>
                                                ...
                                                data
                                                    ...
                                                    .map((item, index) => (
                                                        <tr key={index} style={{ ...generateStyleTableCol(this.tableBody, (data || []).length) }} className="table-hover-css">
                                                            ...
                                                            <td style={{ textAlign: 'right' }} className='table-scale-col'>{item.name}</td>
                                                            <td style={{ textAlign: 'right' }} className='table-scale-col'>{item.countCompany || 0}</td>
                                                            <td style={{ textAlign: 'right' }} className='table-scale-col'>{item.countBelong || 0}</td>
                                                            <td style={{ textAlign: 'right' }} className='table-scale-col'>{item.countNotBelong || 0}</td>
                                                        </tr>
                                                    ))
                                            )
                                        }
