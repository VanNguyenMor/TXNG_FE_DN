...
import { REPORT_ZONING_COMPANY } from "../../../helpers/constant";
...
class ReportZoningCompany extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: null,
            district: null,
            ward: null,
            headerTitle: REPORT_ZONING_COMPANY,
            page: 0,
            limit: LIMITS.reportV2ListZoningPlantingZoneByCompany,
            districtId: '',
            wardId: '',
            taxCode: '',
            companyName: '',
    ...
    componentWillMount() {
        const { getDistrictList, getListPlantingZone } = this.props;

        getListPlantingZone({}).then(res => {
            const plantingZones = ((res.data || {}).data || {}).plantingZones || [];
    ...
    handleSelectPlantingZone = value => {
        const { getWardList } = this.props;
        let { yearM, yearQ, isChecked, dataYear } = this.state;
        if (value == null) { value = '' }

        this.setState({ plantingZoneId: value })
        // if (isChecked == 1) {
        //   this.fetchSummary(yearM + '&districtId=' + value);
        // } else if (isChecked == 2) {
        //   this.fetchSummaryQuarter(yearQ + '&districtId=' + value)
    ...
    onFilter = () => {
        const { fromDate, toDate, taxCode, companyName, plantingZoneId, districtId, wardId, page, limit } = this.state;
        const { getReportListZoningPlantingZoneByCompany } = this.props;
        ...
        getReportListZoningPlantingZoneByCompany(body).then(res => {
    ...
    onExcel = () => {
        const { fromDate, toDate, taxCode, companyName, plantingZoneId, districtId, wardId, page, limit } = this.state;
        const { printExcelReportListZoningPlantingZoneByCompany } = this.props;
        ...
        printExcelReportListZoningPlantingZoneByCompany(body).then(res => {
            ...
            if (ok) {
                ...
                if (data) {
                    fakeDownloadLinkFile(data, FILE_NAMES.reportV2ListZoningPlantingZoneByCompany());
                ...
                this.setState(previousState => {
                    return {
                        ...previousState,
                        isShowPopupMessage: true,
                        message: 'Không có dữ liệu để xuất tệp tin excel'
                    }
                });
            }
        });
                        ...
                        <Container fluid>
                            <Row responsive={1}>
                                <div className="col-11" >
                                    ...
                                    <div className="row" style={{ display: 'flex', justifyContent: 'space-between', paddingRight: '220px' }}>
                                        <div>
                                            <label className="form-control-label">Vùng</label>
                                            <Select
                                                //name="status"
                                                title='Chọn quận huyện'
                                                ref={ref => this.redSelectPlantingZone = ref}
                                                data={plantingZones}
                                                labelName='name'
                                                val='id'
                                                handleChange={this.handleSelectPlantingZone}
                                            />
                                        </div>
                                        <div>
                            ...
                            <h3 className={`${classes.text} ${classes.textLable}`}> BÁO CÁO QUY HOẠCH VÙNG SẢN XUẤT THEO DOANH NGHIỆP</h3>
                                                ...
                                                data
                                                    ...
                                                    .map((item, index) => (
                                                        <tr key={index} style={{ ...generateStyleTableCol(this.tableBody, (data || []).length) }} className="table-hover-css">
                                                            ...
                                                            <td style={{ textAlign: 'left' }} className='table-scale-col'>{item.plantingZoneName}</td>
                                                            <td style={{ textAlign: 'left' }} className='table-scale-col'>{item.taxCode}</td>
                                                            <td style={{ textAlign: 'left' }} className='table-scale-col'>{item.companyName}</td>
                                                            <td style={{ textAlign: 'left' }} className='table-scale-col'>{item.address}</td>
                                                            ...
                                                            <td style={{ textAlign: 'left' }} className='table-scale-col'>{item.belongOnTotal}</td>
                                                        </tr>
                                                    ))
                                            )
                                        }
