...
class UpdateModal extends Component {
  ...
  async componentWillMount() {
    const { requestGetProducts, requestListPartner, id } = this.props;
    let { data } = this.state;

    if (id) {
      await requestGetProducts(id).then(
        res => {
          const data = (res || {}).data || {};
          ...
          if (dataProducts.verifiedImage) {
            const pIm = dataProducts.verifiedImage;
            const spl = pIm.split(';');
          ...
          data.VerifyID = dataProducts.verifyID;
          ...
          this.setState(previousState => {
            return {
              ...
              id,
              data,
              fileImage: dataProducts.verifiedImage
            }
    ...
    if (files.length > 0) {
      ...
      if (file) {
        ...
        if (this.state.mfileImg.length > 0) {
          let mfileImg = [...this.state.mfileImg];
          for (let i = 0; i < files.length; i++) {
            mfileImg.push(new File([files[i]], files[i].name, { type: files[i].type }));
          }
          this.setState(previousState => {
            return {
              ...previousState,
        ...
        if (id) {
          if (pathImageDefaul) {
            this.setState(previousState => {
              return {
                ...previousState,
                pathImageDefaul: pathImageDefaul.concat(pathImage)
              }
            });
  ...
  render() {
    const { dataPartners, pathImageDefaul, dataProducts, verifyIDChange, errors } = this.state;
    const { errorUpdate } = this.props;

    let _dataPartners
    let _dataPartnersName

    if (dataProducts.verifyID != null) {
      _dataPartners = dataPartners.filter(x => dataProducts.verifyID == x.id)
      ...
      <div className={classes.formControl}>
        ...
        <div className={classes.rowItem} style={{ justifyContent: 'flex-start' }}>
          <Select
            ...
            data={dataPartners}
            defaultValue={dataProducts.verifyID}
