...
import { PRODUCTS_VERIFY } from "../../../helpers/constant";
...
import {
  ...
  Spinner,
  Button,
  ButtonDropdown, DropdownToggle, DropdownMenu, DropdownItem, Input
} from "reactstrap";
import moment from "moment";

class ProductsVerify extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
    ...
    this.redSelect = null;
    this.redSelectCom = React.createRef();
    this.redSelectSta = null;
  }
        ...
        if (typeof (data) !== 'undefined') {
          ...
          if (typeof (data.listVerify) !== 'undefined') {
            ...
            if (data.listVerify !== null) {
              ...
              if (typeof (data.listVerify.products) !== 'undefined') {
                if (data.listVerify.products == '') {
                  data.listVerify.products = []
                }
                data.listVerify.products.map((item, key) => {
                ...
                this.setState({
                  data: data.listVerify.products,
                  history: data.listVerify.products,
                  listLength: data.listVerify.products.length,
                  totalPage: Math.ceil(data.listVerify.products.length / limit),
                  isLoaded: data.isLoading,
                  status: data.status,
                  message: PLEASE_CHECK_CONNECT(data.message),
                  totalElement: data.listVerify.products.length > limit ? limit : data.listVerify.products.length
                });
              } else {
                this.setState({
                  history: data.listVerify.products,
  ...
  fetchSummary = (data) => {
    const { requestListProductsVerify } = this.props;

    requestListProductsVerify(data);
  }
  ...
  handleUploadProducts = () => {
    const { newData, idCurRow, filter, mfileImg, fileImage,
      currentVerifiedStatus, currentCompanyID, currentFieldID
    } = this.state;
    const { requestUploadFileProducts } = this.props;
    const errorUpdate = {};
    ...
    requestUploadFileProducts(formData).then(res => {
      if (res.data.status == 200) {
        this.fetchSummary({
          "fieldID": currentFieldID ? currentFieldID : '',
          "productCode": "",
          "productName": "",
          "companyID": currentCompanyID ? currentCompanyID : '',
          "verifiedStatus": currentVerifiedStatus ? currentVerifiedStatus : null,
          "orderBy": "",
          "page": null,
          "limit": null
        });
        ...
        this.toggleModal('updateModal');
        toast.success('Upload hồ sơ xác thực sản phẩm thành công!');
      } else {
  ...
  handleDeleteUploadProducts = () => {
    const { idCurRow, currentFieldID, currentCompanyID, currentVerifiedStatus } = this.state;

    const { requestDeleteFileProducts } = this.props;
    if (idCurRow) {
      requestDeleteFileProducts(idCurRow).then((res) => {
        if (res.data.status == 200) {
          this.toggleModal('updateModal');
          ...
          toast.success('Huỷ hồ sơ thành công!')
        } else {
          this.setState({ errMesageFail: res.message });
          this.toggleModal('popupMessage')
        }
  ...
  handleVerifyProducts = () => {
    const { idCurRowVerify, currentFieldID, currentCompanyID, currentVerifiedStatus } = this.state;
    const { requestVerifyProducts } = this.props;
    if (idCurRowVerify) {
      requestVerifyProducts(idCurRowVerify).then(res => {
        // console.log(res);
        if (res.status == 200) {
          this.fetchSummary({
            "fieldID": currentFieldID ? currentFieldID : '',
  ...
  onSelectChange = (value, name) => {
    ...
    if (name == 'verifiedStatus') {
      value = parseInt(value);
      if (value) {
        this.setState({ currentVerifiedStatus: value })
      }
    }
    filter[name] = value;
    ...
    if (name == 'companyID') {
      ...
      this.setState({ currentFieldID: null })
      if (!value || value == '') {
        filter.fieldID = '';
        this.setState({ data: [] })
      } else {
  ...
  render() {
    ...
    if (JSON.parse(localStorage.getItem('IS_ADMIN'))) {
      ...
      ACCOUNT_CLAIM_FF.filter(x => x == "ProductVerified.Verify").map(y => isDisableVerify = false)
      ACCOUNT_CLAIM_FF.filter(x => x == "ProductVerified.Upload").map(y => isDisableUpload = false)
    }
                    ...
                    <div className="col">
                      ...
                      <HeaderTable
                        dataReload={() => {
                          ...
                          if (this.redSelectSta) {
                            this.redSelectSta.resetValue();
                          }
                          if (this.redSelect) {
                            this.redSelect.resetValue();
                          }
                              ...
                              <div className="mg-div-search w_input">
                                ...
                                <div>
                                  <Select
                                    ...
                                    handleChange={value => this.onSelectChange(value, 'verifiedStatus')}
                                  />
                                </div>
                              </div>
                      ...
                      <Card className="shadow">
                        <Table className="align-items-center tablecs table-css-productsVerify" responsive >
                                ...
                                data
                                  ...
                                  .map((item, key) => (
                                    <tr key={key} className="table-hover-css" >
                                      ...
                                      <td style={{ textAlign: 'center' }} className='table-scale-col productsVerify-img'>{item.thumbnail}</td>
