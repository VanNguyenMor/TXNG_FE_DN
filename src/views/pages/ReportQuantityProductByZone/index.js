...
import { REPORT_QUANTITY_PRODUCT_BY_ZONE } from "../../../helpers/constant";
...
class ReportQuantityProductByZone extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: null,
            district: null,
            ward: null,
            headerTitle: REPORT_QUANTITY_PRODUCT_BY_ZONE,
            productGroups: [],
            page: 0,
            limit: LIMITS.reportV2ListQuantityProductByPlantingZone,
            districtId: '',
            wardId: '',
            productGroupId: '',
            plantingZoneId: '',
    ...
    onFilter = () => {
        const { fromDate, toDate, productGroupId, districtId, wardId, page, limit } = this.state;
        const { getReportListQuantityProductByPlantingZone } = this.props;
        ...
        getReportListQuantityProductByPlantingZone(body).then(res => {
    ...
    onExcel = () => {
        const { fromDate, toDate, productGroupId, districtId, wardId, page, limit } = this.state;
        const { printExcelReportListQuantityProductByPlantingZone } = this.props;
        ...
        printExcelReportListQuantityProductByPlantingZone(body).then(res => {
            ...
            if (ok) {
                ...
                if (data) {
                    fakeDownloadLinkFile(data, FILE_NAMES.reportV2ListQuantityProductByPlantingZone());
            ...
            <>
                {
                    <div className={classes.wrapper}>
                        <Container fluid>
                            ...
                            <h3 className={`${classes.text} ${classes.textLable}`}> BÁO CÁO SẢN LƯỢNG HÀNG HÓA THEO VÙNG</h3>
