...
import { actionCompanyListExpiring } from "../../../actions/CompanyListExpiringActions";
...
import { COMPANY_LIST_EXPIRING_HEADER, COMPANY_LIST_EXPIRING_HEADER_SEARCH } from "../../../helpers/constant";
...
// import KHOA from "../../../assets/images/buttons/KHOA.png";
import ExpiredIcon from "../../../assets/img/buttons/GIAHAN.png";
import LockIcon from "../../../assets/img/buttons/KHOA.png";
...
class CompanyListAwaitExpired extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
      ...
      history: [],
      dataMaping: [
        'index', 'registeredDate', 'expiredDate', 'fieldName', 'companyName', 'taxCode', 'phoneNumber', 'address',
      ],
      searchData: [],
      filterList: [],
      checkAtive: [{}],
      ngayhethan: new Date(),
      headerTitle: COMPANY_LIST_EXPIRING_HEADER,
  ...
  componentWillReceiveProps(nextProp) {
    ...
    const { requestCompanyListExpiring } = nextProp;

    if (fieldData !== this.state.field) {
      if (typeof (fieldData) !== 'undefined') {
        if (fieldData.field !== null) {
          if (typeof (fieldData.field) !== 'undefined') {
            this.setState({ field: fieldData.field.fields, isLoaded: data.isLoading, status: data.status, message: PLEASE_CHECK_CONNECT(data.message) });
          } else {
            this.setState({ field: fieldData.field, isLoaded: data.isLoading, status: data.status, message: PLEASE_CHECK_CONNECT(data.message) });
          }
        }
      }
    }
          ...
          if (data.company !== null) {
            if (typeof (data.company.companies) !== 'undefined') {
              ...
              data.company.companies.map((item, key) => {
                let registeredDate = item.registeredDate;
                let expiredDate = item.expiredDate;
                item['index'] = key + 1;
                item['collapse'] = false;
                // item['registeredDate'] = moment(registeredDate).format('DD/MM/YYYY');
                // item['expiredDate'] = moment(expiredDate).format('DD/MM/YYYY');
              });
              this.setState({
    ...
    if (typeof (data.extend) !== 'undefined') {
      if (data.status && !fetchingUnExtend) {
        this.setState({ data: [] });
        requestCompanyListExpiring(JSON.stringify({
          "fieldID": "",
          "comapanyName": "",
          "taxCode": "",
          "phone": "",
  ...
  fetchSummaryLock = (id) => {
    const { requestCompanyLock } = this.props;

    requestCompanyLock(id);
  }
  componentDidUpdate() {
  ...
  fetchSummary = (data) => {
    const { requestCompanyListExpiring } = this.props;

    requestCompanyListExpiring(data);
  }
  ...
  handleCloseBL = (value) => {
    const { openBL } = this.state;

    this.setState({ openBL: value });
  }
  // buttonAcitveArea = (ele) => {
  //   const { classes } = this.props;
  //   const { open, openBL } = this.state;
  ...
  //       <div className='edit-item'
  //         onClick={() => {
  //           this.handleCloseBL(true);
  //           this.setState({ block: ele });
  //         }}
  //         open={openBL}
  //       >
  //         <img src={KHOA} alt='khoa' title='Khóa' />
  //       </div>
  //     </div>
  //   );
  // }
  ...
  handleLockRow = () => {
    const { requestCompanyLock, requestCompanyListExpiring } = this.props;
    let { data, lockItem } = this.state;
    let newData = data.filter(item => item.id === lockItem).map((item, key) => {
      item.status = 0
    });

    requestCompanyLock(lockItem)
      .then(res => (
        res.status === 200 ? (
          // this.setState({
          //   comfirm: res.data,
  ...
  render() {
    ...
    if (IS_ADMIN) {
      ...
      ACCOUNT_CLAIM_FF.filter(x => x == "ExtensionCompanies.Extend").map(y => isDisableExtend = false)
      ACCOUNT_CLAIM_FF.filter(x => x == "ExtensionCompanies.Lock").map(y => isDisableLock = false)
    }
                                ...
                                data
                                  ...
                                  .map((item, key) => (
                                    <tr key={key} style={{ ...generateStyleTableCol(this.tableBody, (data || []).length) }}>
                                      ...
                                      <td style={{ textAlign: 'center' }} className='table-scale-col'>{moment(item.expiredDate).format('DD/MM/YYYY')}</td>
                                      ...
                                      {/* <td>
                                        <div className={classes.editArea}>
                                          ...
                                          >
                                            <img src={ExpiredIcon} alt="Gia hạn" title="Gia hạn" />
                                          </div>
                                          <div className={classes.item}
                                            onClick={() => {
                                          ...
                                          >
                                            <img src={LockIcon} alt="Khóa" title="Khóa" />
                                          </div>
                                        </div>
                                      </td> */}
                                      <td>
                                        {isDisableExtend == true && isDisableLock == true ? null : (
              ...
              <WarningPopup
                ...
                moduleBody={
                  <p style={{ textAlign: 'center', fontSize: '1.2rem' }}>
                    Bạn đồng ý khóa doanh nghiệp này?
                  </p>}
...
const mapStateToProps = (state) => {
  return {
    company: state.CompanyListExpiringStore,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...bindActionCreators(actionCompanyListExpiring, dispatch),
