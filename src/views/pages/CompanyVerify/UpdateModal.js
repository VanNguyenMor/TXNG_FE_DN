...
class UpdateModal extends Component {
  ...
  async componentWillMount() {
    const { requestCompanyGetDetails, requestListPartner, id } = this.props;
    let { data } = this.state;

    if (id) {
      await requestCompanyGetDetails(id).then(
        res => {
          const data = (res || {}).data || {};
          ...
          if (dataCompany.verifiedImage) {
            const pIm = dataCompany.verifiedImage;
            const spl = pIm.split(';');
          ...
          data.ID = dataCompany.id;
          data.VerifyID = dataCompany.verifyID;
  ...
  render() {
    const { dataPartners, pathImageDefaul, dataCompany, verifyIDChange, errors } = this.state;
    const { errorUpdate } = this.props;

    let _dataPartners
    let _dataPartnersName

    if (dataCompany.verifyID != null) {
      _dataPartners = dataPartners.filter(x => dataCompany.verifyID == x.id)
      ...
      <div className={classes.formControl}>
        ...
        <div className={classes.rowItem} style={{ justifyContent: 'flex-start' }}>
          <Select
            ...
            data={dataPartners}
            defaultValue={dataCompany.verifyID}
