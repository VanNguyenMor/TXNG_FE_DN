...
import { PARTNER_TRANFORM_VERIFY } from "../../../helpers/constant";
...
import './PartnerTranformVerify.css';
...
class PartnerTranformVerify extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
      ...
      headerTitle: PARTNER_TRANFORM_VERIFY,
  ...
  componentWillReceiveProps(nextProp) {
    ...
    if (data !== this.state.data) {
      if (typeof (data) !== 'undefined') {
        if (typeof (data.listTranVerify) !== 'undefined') {
          if (data.listTranVerify !== null) {
            if (typeof (data.listTranVerify.partners) !== 'undefined') {
              data.listTranVerify.partners.map((item, key) => {
              ...
              this.setState({
                data: data.listTranVerify.partners,
                history: data.listTranVerify.partners,
                listLength: data.listTranVerify.total,
                totalPage: Math.ceil(data.listTranVerify.partners.length / limit),
                isLoaded: data.isLoading,
                status: data.status,
                message: PLEASE_CHECK_CONNECT(data.message),
                totalElement: data.listTranVerify.total > limit ? limit : data.listTranVerify.total
              });
            } else {
              this.setState({
                history: data.listTranVerify.partners,
  ...
  componentDidMount() {
    /* Fetch Summary */
    const { requestCompanyAll, requestFieldStore, } = this.props;
  ...
  fetchSummary = (data) => {
    const { requestListTranformPartnerVerify } = this.props;

    requestListTranformPartnerVerify(data);
  }
  ...
  handleUploadProducts = () => {
    ...
    const { requestUploadFilePartner } = this.props;
    const errorUpdate = {};
    ...
    requestUploadFilePartner(formData).then(res => {
      if (res.data.status == 200) {
        ...
        this.toggleModal('updateModal');
        toast.success('Upload hồ sơ nhà vận chuyển thành công!');
      } else {
  ...
  handleDeleteUploadProducts = () => {
    ...
    const { requestDeleteFilePartner } = this.props;
    if (idCurRow) {
      requestDeleteFilePartner(idCurRow).then((res) => {
        if (res.data.status == 200) {
          this.toggleModal('updateModal');
          ...
          toast.success('Huỷ hồ sơ nhà vận chuyển thành công!');
        } else {
          this.setState({ errMesageFail: res.message });
          this.toggleModal('popupMessage')
        }
  ...
  handleVerifyProducts = () => {
    ...
    const { requestVerifyPartner } = this.props;
    if (idCurRowVerify) {
      requestVerifyPartner(idCurRowVerify).then(res => {
        if (res.status == 200) {
          this.fetchSummary({
            "fieldID": "",
          ...
          this.toggleModal('updateModal2');
          toast.success('Xác thực nhà vận chuyển thành công!');
        } else {
          this.setState({ errMesageFail: res.message });
          this.toggleModal('popupMessage')
        }
  ...
  render() {
    ...
    if (JSON.parse(localStorage.getItem('IS_ADMIN'))) {
      ...
      ACCOUNT_CLAIM_FF.filter(x => x == "SupplierVerified.Verify").map(y => isDisableVerify = false)
      ACCOUNT_CLAIM_FF.filter(x => x == "SupplierVerified.Upload").map(y => isDisableUpload = false)
    }
                  ...
                  <Row>
                    <div className="col">
                      ...
                      <Card className="shadow">
                        <Table className="align-items-center tablecs table-css-partnerTransformVerify" responsive >
                                ...
                                data
                                  ...
                                  .map((item, key) => (
                                    <tr key={key} className="table-hover-css">
                                      ...
                                      <td style={{ textAlign: 'center' }} className='table-scale-col partnerTransformVerify-img'>{item.thumbnail}</td>
                                      <td style={{ textAlign: 'left', whiteSpace: 'normal' }}>
                                        <span style={{ color: '#09b2fd' }}><b>{item.partnerName}</b></span><br />
