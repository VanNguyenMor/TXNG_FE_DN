...
import {
  PLEASE_CHECK_CONNECT,
  PLEASE_CHECK_CONNECT_SUCCESS,
...
import { Card, Table, Container, Row, Spinner } from "reactstrap";
...
class Permission extends Component {
          ...
          if (data.data !== null) {
            if (typeof data.data.gridView !== "undefined") {
              ...
              removeDuplicates(
                ...
                data.data.gridView
                  .filter((x) => x.functionID == item.functionID)
                  .map((item1) => {
                    datapermission
                      .filter(
                        (x) =>
                          item1.functionID == x.functionID &&
                          item1.actionID == x.actionID
                      )
                      .map((itemXXX) => {
              ...
              dataAllFilter = removeDuplicates(
                dataOneLine,
                (item) => item.functionID
              );
              // Get GridView
  ...
  render() {
    ...
    if (roles) {
      if (!dataRoleChange) {
        crrRoles = (roles || []).filter(
          (x) => localStorage.getItem("ACCOUNT_ID") == x.id
        );
    ...
    if (JSON.parse(localStorage.getItem("IS_ADMIN"))) {
      ...
      ACCOUNT_CLAIM_FF.filter((x) => x == "Permissions.Add").map(
        (y) => (isDisableAdd = false)
      );
      ACCOUNT_CLAIM_FF.filter((x) => x == "Permissions.Delete").map(
                  ...
                  <Row>
                    <div className="col">
                      ...
                      <Card className="shadow">
                        <Table
                          className={`align-items-center ${classes.table} table-css-permission`}
                          responsive
                        >
                          <HeadTitleTable
                              ...
                              <tbody key={key}>
                                ...
                                </tr>
                                {
                                  ...
                                  dataAllFilter
                                    .filter(
                                      (el) =>
                                        el.parentID.indexOf(item.parentID) > -1
                                    )
                                    .map((el, index) => {
                                      return (
                                        <tr
                                          key={index}
                                          className="table-hover-css permission-css-check"
                                        >
